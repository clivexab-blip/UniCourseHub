...
class AdminProfileFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            ...
            tvRole.text = getString(com.unicoursehub.app.R.string.role_administrator)
            tvStat1Value.text = db.countAllCourses().toString()
            tvStat1Label.text = getString(com.unicoursehub.app.R.string.stat_courses)
            tvStat2Value.text = db.getAllProjects().size.toString()
            tvStat2Label.text = getString(com.unicoursehub.app.R.string.stat_projects)
            tvStat3Value.text = db.countAllUsers().toString()
            tvStat3Label.text = getString(com.unicoursehub.app.R.string.stat_users)
            tvEmail.text = user.email
            tvStudentId.text = user.studentId.ifEmpty { "—" }
            tvRoleRow.text = getString(com.unicoursehub.app.R.string.role_admin)
